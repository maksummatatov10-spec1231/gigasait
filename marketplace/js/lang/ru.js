...
window.MARKET_LANG.ru = {
  ...
  ui: {
    ...
    trStatus: 'Переводчик', trWorking: 'Переводим… {n}', trDone: 'Переведено', trOff: 'Автоперевод выключен', trFailed: 'Перевод недоступен', trFailedHint: 'Сервер переводчика не отвечает (нет интернета, блокировка сети или расширения браузера). Показан оригинал на русском.', trRetry: 'Повторить', trDiag: 'Диагностика перевода', trProvider: 'Сервис', trProviderNone: 'ещё не подключался', trLastError: 'Последняя ошибка', trNoErrors: 'ошибок нет', trStats: 'Переведено пакетов: {ok}, ошибок: {fail}, в кэше: {cache}', trTest: 'Проверить перевод', trTesting: 'Проверяем…', trTestOk: 'Перевод работает ({provider}): «{sample}»', trTestFail: 'Не удалось: {error}', trOnlyNonRu: 'Перевод нужен только при не-русском языке интерфейса', trTranslated: 'переведено', trOriginal: 'оригинал', autoDarkNote: 'Браузер принудительно затемняет сайты (Chrome: Настройки → Тема → «Применять тёмную тему к сайтам»; Яндекс.Браузер / Samsung Internet: «Тёмный режим для сайтов»). Из-за этого светлая тема выглядит тёмной. Выключите эту настройку в браузере.',
